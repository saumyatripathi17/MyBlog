...
const BlogTableItem = ({ blog, fetchBlogs, index }) => {
  ...
  const deleteBlog = async () => {
    ...
    try {
      const { data } = await axios.post('/api/admin/delete-blog', { blogId: _id });
      if (data.success) {
  ...
  const togglePublish = async () => {
    try {
      const { data } = await axios.post('/api/admin/toggle-publish', { id: _id });
      if (data.success) {
        toast.success(data.message);
        fetchBlogs();
      } else {
    ...
    <tr className="hover:bg-blue-50 transition-colors duration-200 border-b border-gray-100">
      ...
      <td className="px-6 py-4 flex gap-3 justify-center">
        <button
          ...
          className="border px-3 py-1 rounded hover:bg-gray-100 transition-all"
        >
